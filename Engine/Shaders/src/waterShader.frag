...
void main()
{
    ...
    outColor = mix(uboWater.mDeepColor, uboWater.mShallowColor, inPosition.y/1.0);
    outSpecularColor = vec4(0.5, 0.5, 0.5, 0.5);
	...
	outMetallic = 0.0; //texture(ormSampler, inTexcoord).b; // uboGeometry.mMetallic;
	outRoughness = 0.5; //texture(ormSampler, inTexcoord).g; // uboGeometry.mRoughness;
}