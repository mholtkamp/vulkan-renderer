...
class WaterSimulationPipeline : public Pipeline
{
public:

	WaterSimulationPipeline()
	{
		mComputePipeline = true;
		mComputeShaderPath = ENGINE_SHADER_DIR "waterSimulationShader.comp";
	...
	virtual void PopulateLayoutBindings() override
	{
		...
		PushSet();
		AddLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_COMPUTE_BIT); // Wave Particle Buffer
		AddLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_COMPUTE_BIT); // Displacement Map Image
	}
};

class WaterRenderingPipeline : public GeometryPipeline
{
public:

	WaterRenderingPipeline()
	{
		mVertexShaderPath = ENGINE_SHADER_DIR "waterShader.vert";
		mFragmentShaderPath = ENGINE_SHADER_DIR "waterShader.frag";
		mCullMode = VK_CULL_MODE_NONE;
	...
	virtual void PopulateLayoutBindings() override
	{
		GeometryPipeline::PopulateLayoutBindings();

		PushSet();
		AddLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER , VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT); // Water Uniform Buffer
		AddLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT); // Displacement Map Image
	}
};